use crate::commands::session_commands::{terminate_session, send_quit_command_to_session};
...
async fn terminate_session_accepts_session_id_and_succeeds_when_missing() {
    ...
    assert!(res.is_ok(), "terminate_session should succeed even if session is missing");
}
...
async fn send_quit_command_uses_session_id_and_errors_when_missing() {
    ...
    assert!(res.is_err(), "send_quit_command_to_session should error for missing session");
    let msg = res.unwrap_err();
    assert!(msg.contains("Session not found"), "Unexpected error message: {}", msg);
}
