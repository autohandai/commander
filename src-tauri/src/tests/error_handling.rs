...
mod tests {
    ...
    fn test_project_error_creation() {
        ...
        let user_msg = error.user_message();
        assert_eq!(user_msg, "Project operation 'create' failed for 'MyProject': Directory already exists");
    }
    ...
    fn test_llm_error_creation() {
        ...
        let user_msg = error.user_message();
        assert_eq!(user_msg, "OpenRouter operation 'fetch_models' failed: API key required");
    }
    ...
    fn test_network_error_with_status_code() {
        let error = CommanderError::network("https://api.example.com", Some(404), "Resource not found");
    ...
    fn test_session_error_with_id() {
        let error = CommanderError::session(Some("sess-123".to_string()), "terminate", "Session not found");
    ...
    fn test_error_display_trait() {
        ...
        assert_eq!(displayed, "Configuration error in Settings: Invalid JSON format");
    }
    ...
    fn test_error_serialization() {
        ...
        let deserialized: CommanderError = serde_json::from_str(&json).expect("Error should be deserializable");
        match deserialized {
