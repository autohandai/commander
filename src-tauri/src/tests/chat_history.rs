...
mod tests {
    ...
    use crate::services::chat_history_service::{
        ...
        save_chat_session, load_chat_sessions, load_session_messages,
        delete_chat_session, migrate_legacy_chat_data,
    ...
    fn create_test_message(role: &str, content: &str, agent: &str, timestamp_offset: i64) -> EnhancedChatMessage {
    ...
    async fn test_create_commander_directory() {
        ...
        assert!(result.is_ok(), "Should create .commander directory successfully");

        let commander_dir = temp_dir.path().join(".commander").join("chat_history");
        assert!(commander_dir.exists(), ".commander/chat_history directory should exist");
        assert!(commander_dir.is_dir(), ".commander/chat_history should be a directory");
    }
    ...
    async fn test_session_grouping_by_time_gap() {
        ...
        assert_eq!(sessions.len(), 2, "Should create 2 sessions due to time gap");
    ...
    async fn test_agent_specific_session_grouping() {
        ...
        let sessions = group_messages_into_sessions(messages).await.unwrap();
        assert_eq!(sessions.len(), 2, "Should create 2 sessions for different agents");
    ...
    async fn test_save_and_load_chat_session() {
        ...
        assert!(loaded_messages.is_ok(), "Should load session messages successfully");
    ...
    async fn test_sessions_index_management() {
        ...
        // Save first session
        let sessions1 = group_messages_into_sessions(messages1.clone()).await.unwrap();
        save_chat_session(&project_path, &sessions1[0], &messages1).await.unwrap();

        // Save second session
        let sessions2 = group_messages_into_sessions(messages2.clone()).await.unwrap();
        save_chat_session(&project_path, &sessions2[0], &messages2).await.unwrap();
    ...
    async fn test_agent_filtering() {
        ...
        let codex_messages = vec![
            create_test_message("user", "Codex message", "codex", 600),
        ];

        // Save sessions
        let claude_sessions = group_messages_into_sessions(claude_messages.clone()).await.unwrap();
        save_chat_session(&project_path, &claude_sessions[0], &claude_messages).await.unwrap();

        let codex_sessions = group_messages_into_sessions(codex_messages.clone()).await.unwrap();
        save_chat_session(&project_path, &codex_sessions[0], &codex_messages).await.unwrap();

        // Filter by agent
        let claude_only = load_chat_sessions(&project_path, None, Some("claude".to_string())).await.unwrap();
        assert_eq!(claude_only.len(), 1);
        assert_eq!(claude_only[0].agent, "claude");

        let codex_only = load_chat_sessions(&project_path, None, Some("codex".to_string())).await.unwrap();
    ...
    async fn test_session_limit() {
        ...
        for i in 0..5 {
            let messages = vec![
                create_test_message("user", &format!("Message {}", i), "claude", i * 600),
            ];
            let sessions = group_messages_into_sessions(messages.clone()).await.unwrap();
        ...
        // Test limit
        let limited = load_chat_sessions(&project_path, Some(3), None).await.unwrap();
    ...
    async fn test_delete_session() {
        ...
        let messages = vec![
            create_test_message("user", "To be deleted", "claude", 0),
        ];
        ...
        assert!(session_file_result.is_err(), "Session file should be deleted");
    }
    ...
    async fn test_file_mentions_extraction() {
        ...
        assert!(message.metadata.file_mentions.contains(&"src/main.rs".to_string()));
        assert!(message.metadata.file_mentions.contains(&"tests/mod.rs".to_string()));
    }
    ...
    async fn test_session_summary_generation() {
        ...
        let messages = vec![
            create_test_message("user", "Can you help me implement a sorting algorithm?", "claude", 0),
            create_test_message("assistant", "I'll help you implement quicksort", "claude", 60),
        ];

        let sessions = group_messages_into_sessions(messages).await.unwrap();
        let session = &sessions[0];

        assert_eq!(session.summary, "Can you help me implement a sorting algorithm?");
    }
    ...
    async fn test_migration_from_legacy_format() {
        ...
        assert_eq!(sessions.len(), 1, "Should create one session from legacy data");
        assert_eq!(sessions[0].agent, "claude");
    ...
    async fn test_cross_platform_file_handling() {
        ...
        let messages = vec![
            create_test_message("user", "Cross platform test", "claude", 0),
        ];
    ...
    async fn test_large_session_handling() {
        ...
        let loaded = load_session_messages(&project_path, &sessions[0].id).await.unwrap();
