...
mod tests {
    ...
    async fn test_resolve_git_project_path_with_worktree() {
        ...
        assert!(result.is_some(), "Should resolve worktree to main repository or itself");
        
        // The result should be a valid git repository path
        let resolved_path = result.unwrap();
        assert!(Path::new(&resolved_path).exists(), "Resolved path should exist");
    }
    ...
    async fn test_resolve_git_project_path_subdir_of_repo() {
        ...
        assert!(result.is_none(), "Subdirectory without .git should return None");
        
        // But find_git_root should work from the subdirectory
        let root_result = git_service::find_git_root(&subdir_str);
        assert!(root_result.is_some(), "find_git_root should work from subdirectory");
        
        let root = root_result.unwrap();
        assert_eq!(Path::new(&root), project_path, "Should find the main repository root");
    }
}