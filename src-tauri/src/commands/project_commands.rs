...
pub async fn list_recent_projects(app: tauri::AppHandle) -> Result<Vec<RecentProject>, String> {
    ...
    let store = app.store("recent-projects.json")
        .map_err(|e| format!("Failed to access recent projects store: {}", e))?;
    
    match store.get("projects") {
        Some(value) => {
            let projects: Vec<RecentProject> = serde_json::from_value(value)
            ...
            if valid_projects.len() != original_count {
                let _ = store.set("projects", serde_json::to_value(&valid_projects).unwrap_or_default());
                let _ = store.save();
            }
...
pub async fn clear_recent_projects(app: tauri::AppHandle) -> Result<(), String> {
    ...
    store.save().map_err(|e| format!("Failed to save cleared recent projects: {}", e))?;
...
pub async fn create_new_project_with_git(
    ...
    println!("✅ Project '{}' created successfully and added to recent projects", project_name);
    Ok(project_path_str)
}
